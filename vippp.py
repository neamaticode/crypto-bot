...
# ============================================================
import os as _os
TELEGRAM_TOKEN          = _os.environ.get('TELEGRAM_TOKEN',   '8273080160:AAGsGq1C8z15I-AhoaQMcXjaLFlIF556B5U')
TELEGRAM_CHAT_ID        = _os.environ.get('TELEGRAM_CHAT_ID', '-1002039819514')
...
class DatabaseManager:
    ...
    def _conn(self) -> sqlite3.Connection:
        c = sqlite3.connect(self.db_name, timeout=30, check_same_thread=False)
    ...
    def update_signal(self, signal_id: int, updates: Dict) -> None:
        if not updates:
            return
        conn = self._conn()
        try:
            ...
            conn.execute(f'UPDATE signals SET {clause} WHERE id=?', vals)
            conn.commit()
        except Exception as exc:
    ...
    def get_signals_by_period(self, start: datetime, end: datetime) -> List[Dict]:
        conn = self._conn()
        try:
            rows = conn.execute(
    ...
    # ── AI data ─────────────────────────────────────────────
    def save_ai_data(self, features: Dict, target: int) -> None:
        conn = self._conn()
        try:
            conn.execute(
                '''INSERT INTO ai_data
    ...
    def get_ai_training_data(self) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        conn = self._conn()
        try:
            rows = conn.execute(
...
class AIEngine:
    ...
    def train(self) -> bool:
        try:
            X, y = self.db.get_ai_training_data()
            if X is None:
                return False
...
class CryptoSignalBot:
    ...
    def __init__(self) -> None:
        ...
        self.account_balance     = self.db.get_state('account_balance', INITIAL_ACCOUNT_BALANCE)
        self.last_daily_report   = self.db.get_state('last_daily_report',   None)
        self.last_weekly_report  = self.db.get_state('last_weekly_report',  None)
        self.last_monthly_report = self.db.get_state('last_monthly_report', None)
    ...
    # ── Exchange helpers ─────────────────────────────────────
    def _fetch_ohlcv(self, symbol: str, tf: str, limit: int = 500) -> Optional[pd.DataFrame]:
        try:
            ...
            # NOTE: _fetch_ohlcv is called from within an async context via
            # run_in_executor or directly; sleeping here is intentional as this
            # is a synchronous helper.  Callers that are in async paths use
            # 'await asyncio.sleep(0)' around the call-site to yield control.
            time.sleep(15)
            return None
    ...
    def _funding(self, symbol: str) -> float:
        try:
            r = self.exchange.fetch_funding_rate(symbol)
            return float(r.get('fundingRate', 0) or 0)
        except Exception:
            return 0.0

    def _ob_imbalance(self, symbol: str) -> float:
        try:
    ...
    def _futures_symbols(self) -> List[str]:
        try:
    ...
    async def _analyze(self, symbol: str) -> Optional[Dict]:
        try:
            df = self._fetch_ohlcv(symbol, '1h', 500)
            if df is None or len(df) < 200:
                return None
            ...
            for tf in ['15m', '4h', '1d']:
                mtf_df = self._fetch_ohlcv(symbol, tf, 250)
            ...
            ob_imbal   = self._ob_imbalance(symbol)
    ...
    async def _check_reports(self) -> None:
        ...
        if now.hour == 23 and now.minute < 5:
            ...
            if self.last_daily_report != ds:
                ...
                self.db.set_state('last_daily_report', ds)
        ...
        if now.weekday() == 4 and now.hour == 23 and now.minute < 5:
            ...
            if self.last_weekly_report != ws:
                ...
                self.db.set_state('last_weekly_report', ws)
        ...
        if now.day == 1 and now.hour == 23 and now.minute < 5:
            ...
            if self.last_monthly_report != ms:
                ...
                self.db.set_state('last_monthly_report', ms)

    async def _send_daily(self, now: datetime) -> None:
        start = now.replace(hour=0, minute=0, second=0, microsecond=0)
        sigs  = self.db.get_signals_by_period(start, now)
    ...
    async def _send_monthly(self, now: datetime) -> None:
        ...
        sigs  = self.db.get_signals_by_period(start, end)
    ...
    async def _retrain_if_needed(self) -> None:
        if self.ai.needs_retraining():
            ...
            success = await loop.run_in_executor(None, self.ai.train)
            if success:
    ...
    async def run(self) -> None:
        ...
        while True:
            try:
                ...
                if self._active_count() < MAX_OPEN_TRADES:
                    symbols = self._futures_symbols()
                    if symbols:
                            ...
                            try:
                                ...
                                if sig and sig['confidence'] >= MIN_CONFIDENCE:
                                    ...
                                    if self._correlated(symbol, active) >= 2:
                                        ...
                                        continue
                                    sig_id = self.db.save_signal(sig)
                                    if sig_id < 0:
                                        continue
                                    msg_id = await self._send(self.fmt.signal(sig, sig_id))
                                    if msg_id:
                                        self.db.update_signal(sig_id, {'telegram_msg_id': msg_id})
                                    self._cooldowns[symbol] = datetime.now()
                                    emitted += 1
                                    self.db.save_ai_data(
                                        sig['features'],
